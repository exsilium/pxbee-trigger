	...
	zcl_attribute_write_rec_t *write_rec)
{
	...
	if (write_rec->flags & ZCL_ATTR_WRITE_FLAG_READ_RESP)
	{
		...
		{
			///@to_do should we set a flag in write_rec to indicate that the
...
/**
	...
	@to_do Generate error if command is not one of the three expected commands.

	@to_do Update to limit number of status records in response.
*/
zigbee_zcl_debug
int _zcl_write_attributes( zcl_command_t *cmd)
{
